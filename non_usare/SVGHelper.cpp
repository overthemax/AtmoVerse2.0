...
#include <GxEPD2_BW.h>
#include "Hardware.h" // Per PanelType/display
#include <FS.h>
#include <SD.h>
#include <SPI.h>
#include <Arduino.h>
...
bool SVGHelper::begin(int8_t csPin) {
  ...
  if (!SD.begin(csPin)) {
    DEBUG_PRINTLN(F("[SVG] Errore nell'inizializzazione della scheda SD"));
    return false;
  }
  ...
  if (!SD.exists("/www/icons")) {
    if (!SD.mkdir("/www")) {
      DEBUG_PRINTLN(F("[SVG] Impossibile creare la directory /www"));
      return false;
    }
    if (!SD.mkdir("/www/icons")) {
      DEBUG_PRINTLN(F("[SVG] Impossibile creare la directory /www/icons"));
      return false;
    }
  }
  
  DEBUG_PRINTLN(F("[SVG] Inizializzazione completata con successo"));
  initialized = true;
  return true;
}
...
bool SVGHelper::drawWeatherIcon(DisplayType& display, WeatherIcon icon, int x, int y, int size) {
  ...
  if (icon < 0 || icon >= ICON_COUNT) {
    DEBUG_PRINT(F("[SVG] Errore: codice icona non valido: "));
    DEBUG_PRINT(icon);
    DEBUG_PRINTLN('"');
    return false;
  }
...
// Funzione per disegnare un percorso SVG (path) - versione generica
template<typename DisplayType>
...
// Nota: La definizione principale della funzione drawPath si trova sopra alle righe 385-707
// Questa implementazione duplicata è stata rimossa per evitare conflitti di definizione
...
// Rimuoviamo specializzazioni legate a GxEPD2_583_T8 non presenti in alcune versioni della libreria.
// Le versioni template generiche dichiarate in SVGHelper.h verranno usate al loro posto.
// (nessuna implementazione specifica necessaria qui)


// Manteniamo istanziazioni esplicite per i tipi di display utilizzati
// Compila queste istanziazioni solo se il supporto SVG è attivato dal progetto
#if defined(ENABLE_SVG_SUPPORT)
// Istanziazioni basate su PanelType con paginazione 64 (preesistenti)
template bool SVGHelper::drawWeatherIcon<GxEPD2_BW<PanelType, 64>>(
...
template bool SVGHelper::loadSVG<GxEPD2_BW<PanelType, 64>>(
    GxEPD2_BW<PanelType, 64>& display,
    const char* filename,
    int x,
    int y,
...
template bool SVGHelper::drawMoonPhase<GxEPD2_BW<PanelType, 64>>(
    GxEPD2_BW<PanelType, 64>& display,
    int x,
    int y,
    int size,
...
// Istanziazioni esplicite per GxEPD2_583_T8 con paginazione 32 (usato in Display.cpp)
template bool SVGHelper::drawWeatherIcon<GxEPD2_BW<GxEPD2_583_T8, 32>>(
...
template bool SVGHelper::loadSVG<GxEPD2_BW<GxEPD2_583_T8, 32>>(
    GxEPD2_BW<GxEPD2_583_T8, 32>& display,
    const char* filename,
    int x,
    int y,
...
template bool SVGHelper::drawMoonPhase<GxEPD2_BW<GxEPD2_583_T8, 32>>(
    GxEPD2_BW<GxEPD2_583_T8, 32>& display,
    int x,
    int y,
    int size,
