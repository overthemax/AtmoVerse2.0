...
// Reference to the display defined in Hardware.cpp
...
void displaySetupScreen(String apName, String ipAddress) {
  ...
  do {
    ...
    // Cambio da "Modalit├á Configurazione" a "Prima Configurazione"
    display.setFont(&FreeSerif12pt7b);
...
void drawDisplayContent() {
  ...
  display.firstPage();
  do {
    // Disegna il calendario ancora pi├╣ grande in alto a destra
    drawCalendar(display.width() - 170, 5, 165, 170);
  
  // Disegna il nome della citt├á a sinistra
  display.setFont(&FreeSerif12pt7b);
  display.setCursor(20, 40);
  display.print(config.city);
  
  // Disegna solo l'ora sotto la citt├á (non la data)
  ...
  // Mostra l'IP in basso al centro con font pi├╣ grande e debug seriale
  display.setFont(&FreeMonoBold9pt7b); // Font pi├╣ leggibile
  String ipString;
...
void drawWeatherIcon(int x, int y, int weatherId, bool isNight) {
  ...
  if (WeatherIcons::drawWeatherIcon(display, iconCode, x, y, iconSize)) {
    ...
    // separata dall'icona principale (65px pi├╣ in alto, come da preferenze utente)
    if (isNight && (weatherId == 800 || weatherId == 801)) {
      ...
      if (SVGHelper::begin()) {
        ...
        // Fallback per la luna se SVG non ├¿ disponibile
  ...
  // Se non ├¿ stato possibile utilizzare le icone OpenWeatherMap, utilizziamo il fallback SVG
  if (SVGHelper::begin()) {
    ...
    if (SVGHelper::drawWeatherIcon(display, icon, x, y, iconSize)) {
      ...
      if (isNight && (weatherId == 800 || weatherId == 801)) {
        // Disegna la fase lunare (65px pi├╣ in alto, come da preferenze utente)
        SVGHelper::drawMoonPhase(display, iconCenterX - 30, iconCenterY - 65, 40, currentWeather.moon_phase);
      }
      return;
    }
  ...
  // Fallback se n├® OpenWeatherMap n├® SVG sono disponibili
...
void drawQuote(int x, int y, int maxWidth) {
  ...
  const char* quotes[] = {
    ...
    "Non c'├¿ nulla di permanente tranne il cambiamento.",
    "La vita ├¿ ci├▓ che ti accade mentre sei impegnato a fare altri progetti.",
    "La felicit├á non ├¿ qualcosa di pronto all'uso. Viene dalle tue azioni."
  };
