...
- **Monitoraggio Meteo Avanzato**: 
  - **Alert**: Notifiche per temperature estreme, vento forte, temporali e pioggia.
  - **Storico**: Salvataggio dati orari e giornalieri con statistiche settimanali/mensili.
- **Gestione Energetica Intelligente**:
  - **Battery Manager**: Stima precisa durata, calibrazione voltaggio e gestione stati di carica.
  - **Night Mode**: Attivazione automatica tema scuro e riduzione aggiornamenti in ore notturne.
- **Gestione avanzata icone meteo**: Icone SVG/PNG ottimizzate per e-ink, posizionamento personalizzato.
- **Citazioni dinamiche**: Box citazioni adattivo con cambio automatico in base a ora/meteo.
- **Configurazione Smart**: 
  - **QR Code**: Generazione QR per connessione WiFi rapida e accesso configurazione.
  - **Interfaccia Web**: Gestione completa dispositivo da browser.
- **Gestione errori**: Fallback automatico e logging dettagliato.
- **SD Card**: Utilizzata per config, storico dati e assets.
...
- **SD Card**: CS GPIO14, SCK GPIO27, MOSI GPIO26, MISO GPIO25 (bus HSPI separato)
