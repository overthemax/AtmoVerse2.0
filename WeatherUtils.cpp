...
bool getWeatherData() {
  ...
  if (config.lat == 0 && config.lon == 0) {
    ...
    // Inizializza coordinate di default in base alla città 
    if (String(config.city).indexOf("Milano") >= 0) {
      config.lat = 45.4642; 
      config.lon = 9.1900;
    } else if (String(config.city).indexOf("Roma") >= 0) {
      config.lat = 41.9028;
      config.lon = 12.4964;
    } else if (String(config.city).indexOf("Napoli") >= 0) {
      config.lat = 40.8518;
      config.lon = 14.2681;
    } else if (String(config.city).indexOf("Torino") >= 0) {
      config.lat = 45.0703;
      config.lon = 7.6869;
    } else {
  ...
  static const char params[] PROGMEM = "&units=metric&lang=it&appid=";
